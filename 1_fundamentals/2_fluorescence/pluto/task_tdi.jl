...
# ╔═╡ d4bf4b60-9b67-4d2b-a240-1bb948e5e4a5
using Unitful

# ╔═╡ ffbf7163-377a-488e-bca6-c083ad2193c4
using DataFrames, Plots, CSV

# ╔═╡ c530410a-e2b7-11ee-0818-a5afbf024a56
md"""
# 2 Fluorescence: TDI rates
"""
...
begin
	plot(data_SB.wavelength, data_SB.absorption; label="absorption")
	plot!(data_SB.wavelength, data_SB.emission; label="absorption", xaxis =("wavelength (nm)"), yaxis = ("abs / em (normaized)"))
end
...
ϵ = @. data_SB.absorption / ( C * l );

# ╔═╡ dcdccdc4-fb96-4cb7-8c42-e6307aea7402
md"""
### convert emission spectrum to frequency axis
"""
...
frequency = 2π * c_0 ./ (data_SB.wavelength  .* 1u"nm") .|> upreferred
...
F_ω = @. (data_SB.emission  .* 1u"nm^-1").* (data_SB.wavelength * 1u"nm")^2 / c_0 .|> u"Hz^-1"
...
begin
	dω = -1 .* diff(frequency)  # frequency is decreasing
	dω = [dω[1]; dω]   # dublicate first element as diff reduces length by one
end

# ╔═╡ a0edaf63-414f-49ad-b69f-73bbfb5fb9c3
md"""
One could use more fancy integration algorithms, but the data is so smooth, that summing over boxes is enough.
"""
...
k_SB = log(10) .* n_solvent.^2 / (π^2 * c_0^2 * N_A) *  sum(F_ω .* dω) / sum(F_ω .* frequency.^-3 .* dω)  * sum(ϵ ./ frequency .* dω) |> u"ns^-1"
...
md"""
The simplest way to fit a single exponential decay is by a weighted sum. Exclude too long delays, as there is no signal anymore. 
"""

# ╔═╡ 0b73ea86-ac81-4c3b-a226-d43b863d1241
begin 
	x = data_tcspc.delay[data_tcspc.delay .< 20];
	y = data_tcspc.occurence[data_tcspc.delay .< 20];
	tau_TCSPC = sum( x .* y) ./ sum(y)
end

# ╔═╡ 6174c157-e158-4562-a70c-e0a77cf0c716
begin
	plot(data_tcspc.delay, data_tcspc.occurence .+ 1e-10, xaxis=("delay (ns)"), yaxis=("occurence", :log, (1, 1000)); label="data")
	plot!(data_tcspc.delay, @. 1000 * exp(-1 * data_tcspc.delay / tau_TCSPC) ;label="single exp. decay")
end
...
manifest_format = "2.0"
project_hash = "2dd6fe40a9b1e439242d68154deab67d58e5bd3f"
