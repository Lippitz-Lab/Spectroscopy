...
# ╔═╡ d07432b6-c8f3-4620-bc26-c0bb83ece8c1
md"""
# 3 Vibrations: Mirror rule tested at BODIPY
"""
...
begin
	plot(frequency, F_ω; label="emission")
	plot!(frequency, A_ω, xaxis = ("frequency"), yaxis =("Abs, Em (normalized)");  label = "absorption")
end
...
begin
	plot(frequency, μ_em ; label="emission")
	plot!(frequency, μ_abs, xaxis = ("frequency"), yaxis =("dipole moments (normalized)");  label = "absorption")
end
...
begin
	# find intersection of spectra in seleced freq. range
	delta = abs.(μ_em .- μ_abs)
	fmin = 2.5e15u"Hz";
	fmax = 3e15u"Hz";
	fsel = frequency[ (frequency .> fmin) .& (frequency .< fmax)]
	deltasel = delta[ (frequency .> fmin) .& (frequency .< fmax)]
	fmid = fsel[argmin(deltasel)];

	# plot mirrored & shifted spectra 
