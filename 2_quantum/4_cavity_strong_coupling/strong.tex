...
The Rabi model assumed a classical electric field in which the particle nature of light is not relevant. But now we have to consider the quantized nature of light. In quantum mechanics this is often called 'second quantization' and we will briefly look at the main results.

The basic idea is very similar to a quantum mechanical harmonic oscillator, i.e. a series of equidistant states with a lower boundary. We describe the states by a quantum number $n$, starting from $n=0$.  The Hamiltonian reads
\begin{equation}
...
\begin{questions}
  \item In order to describe what kind of experiments do we need to use the second quantization ?
...
\begin{marginfigure}
  ...
  \label{fig:strong_Wrigge_Mollow}
\end{marginfigure}
...
An example of an experimental realization is shown in Fig. \ref{fig:strong_photon_blockade}. The authors used a \ch{InAs} quantum dot embedded in a \ch{GaAs} matrix, which behaves similarly to a two-level system at 930~nm wavelength. The cavity was a photonic crystal: air holes in the \ch{GaAs} membrane lead to a bandgap for photons at this wavelength. Defects in the form of missing or shifted holes lead to a cavity with a high quality factor $Q \approx 10^4$. A resonant, spectrally narrow but pulsed laser ($f_\text{rep} = 80 $~MHz) is reflected at the cavity plus quantum dot. The authors try to find two photons that arrive at two detectors simultaneously, a coincidence. For a laser beam obeying Poisson statistics, the probability of finding two photons with delay $\tau$ is independent of $\tau$. With a pulsed laser, you will only find coincidences with delays $\tau = n \, T = n / f_\text{rep}$.
Ideally, for a photon blockade one should find no coincidences at $\tau = 0$. Here the authors were able to show a drop of about 10\% due to background effects. 
...
\begin{marginfigure}
  ...
  \cite{Faraon2008}.}
  \label{fig:strong_photon_blockade}
\end{marginfigure}
