\renewcommand{\lastmod}{June 4, 2021}
...
Already in the last chapter we discussed the properties of an optical cavity containing a two-level system such as an atom or molecule. Atom and cavity couple because the transition dipole operator interacts with the field of the cavity. In the last chapter we assumed that this coupling is 'strong'. This means that the effect of the coupling dominates other effects. We saw that the atom and the cavity lose their identity.  We found new eigenstates as a symmetric and antisymmetric combination of 'photon in the atom and not in the cavity' and 'photon in the cavity and not in the atom'.  In this chapter we still discuss atoms in cavities, but we relax the coupling strength condition. A 'weak' coupling is sufficient. Atom and cavity no longer lose their identity. The excitation is either in the atom or in the cavity.


Without going into too much detail\footcite{Khitrova06,Pelton19,Thomas20}, let us look at the parameters that decide whether a system is in the weak or the strong coupling limit. Already in the chapter on molecular aggregates we have required that the coupling energy $J$ is greater than the difference in the eigenenergies of the systems to be coupled, i.e.
\begin{equation}
J \gg | E_a - E_b| \quad \text{or} \quad g \gg | \Delta | = | \omega_0 - \omega | \quad .
\end{equation}
While this is still required, it is not sufficient. In many cases, the atom is resonant with the cavity, i.e. $\Delta = 0$. This does not automatically lead to strong coupling. We require that the energy can make one round trip from cavity to  atom and back to cavity without being dissipated. Then one cannot say any more whether the excitation is in the atom or in the cavity and we are in the regime of strong coupling. In this way, the decay rates of atom $\gamma_A$ and cavity $\gamma_C$ come into play. We require for strong coupling
\begin{equation}
...
Spontaneous emission like fluorescence is a tricky problem from a quantum mechanical point of view, which we have tried to avoid until now. The reason is that the electronic excited state is an eigenstate of the Hamiltonian and should be stable in time. Since fluorescence also occurs in the dark, an electric field that couples states in a perturbation operator does not help here. We got around this by either discussing absorption (bright fields that couple states) or by using Einstein coefficients (no quantum mechanics). The quantization of the electric field introduced in the last chapter now allows in principle to study spontaneous emission with the tools of quantum mechanics.
...
\end{equation}
We required the vacuum to contain the ground state energy of $\frac{1}{2} \hbar \omega$ which results in a non-zero  intensity. While the average value of the field is zero, its fluctuations lead to an  non-zero average intensity. These vacuum fluctuations cause spontaneous emission. Spontaneous emission is stimulated emission by vacuum fluctuations.
...
\item How long would it take for an emitted fluorescence photon to be absorbed back into the atom if there were only a single optical mode?
\end{questions}
...
This formalism now allows us to change the environment of the emitter. The simplest is a cavity with only a single optical mode. In this case 
\begin{equation}
...
The Purcell effect can be observed in the fluorescence lifetime of emitters. An increased spontaneous emission rate reduces the lifetime of the excited state. The concept is derived considering atoms in cavities.  Solid-state and nanophotonic realizations have been demonstrated. However, in such experimental realizations, care must be taken that the environment does not also modify the non-radiative rates. Emission quenching would lead to a similar reduction in excited state lifetime, but is not a Purcell effect.
...
If we are only interested in the decay rate or the emitted power, an alternative approach is helpful. We now continuously drive the dipole oscillator and calculate the optical far field as a superposition of the direct emission in the direction of the observer and the emission in other directions, which is then reflected in the direction of the observer. These fields interfere constructively or destructively, depending on their phase relation. The total emitted power is obtained by integration over a suitable surface.

\begin{marginfigure}
  \inputtikz{\currfiledir drexhage_data}  
  \caption{Fluorescence lifetime of \ch{Eu^{3+}} ions in front of a silver mirror, compared to a simple model. Data from \cite{Drexhage74}.}
  \end{marginfigure}
